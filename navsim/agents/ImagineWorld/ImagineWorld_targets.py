...
class ImagineWorldTargetBuilder(AbstractTargetBuilder):
    def __init__(self, 
    ...
    def compute_targets(self, scene: Scene) -> Dict[str, torch.Tensor]:
        ...
        try:
            if self.sim_reward_dict_path is not None:
                ...
                reward_dir_name = str(scene.frames[index].cameras.cam_f0.image_path).split('/')[-3]
        ...
        result['token'] = scene.frames[index].token
        #result['reward_dir_name'] = reward_dir_name
        #import pdb;pdb.set_trace()
