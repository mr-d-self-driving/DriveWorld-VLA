...
from navsim.agents.transfuser.transfuser_backbone_latent import TransfuserBackboneLatent
...
class BEVTextFusion(nn.Module): # 双向对齐机制
    def __init__(self, dim=256, heads=8, use_bidirectional=False):
        super().__init__()

        self.use_bidirectional = use_bidirectional

        # Text → BEV  cross-attention
        self.cross1 = nn.MultiheadAttention(embed_dim=dim, num_heads=heads, batch_first=True)
        ...
        if use_bidirectional:
            # BEV → Text  cross-attention
            self.cross2 = nn.MultiheadAttention(embed_dim=dim, num_heads=heads, batch_first=True)
    ...
    def forward(self, bev, text):
        ...
        """
        # ----------------------------
        # 1) Text → BEV (BEV reads text)
        # ----------------------------
        bev2, _ = self.cross1(query=bev, key=text, value=text)
        bev = bev + bev2                  # residual
        bev = bev + self.ff1(self.ln1(bev))
        ...
        # 2) BEV → Text (optional)
        # ----------------------------
        text2, _ = self.cross2(query=text, key=bev, value=bev)
        text = text + text2
        text = text + self.ff2(self.ln2(text))

        return bev, text

class PerceiverCrossAttention(nn.Module):
    def __init__(self, latent_dim=256, latent_cnt=700, input_dim=256, n_heads=8):
        super().__init__()

        # 700 × 256 learnable latent vectors
        self.projection_layer = nn.Linear(1536, latent_dim)

        self.latents = nn.Parameter(torch.randn(latent_cnt, latent_dim))
    ...
    def forward(self, text_tokens):
        """
        text_tokens: [B, 2800, 256]
        Return: [B, 700, 256]
        """
        text_tokens = self.projection_layer(text_tokens)

        B = text_tokens.shape[0]

        # Expand latents for batch: [B, 700, 256]
        latents = self.latents.unsqueeze(0).expand(B, -1, -1)

        # Cross-Attention:
        #   Queries = latents
        #   Keys/Values = text tokens
        # Output: updated_latents [B, 700, 256]
        updated_latents, _ = self.cross_attn(
            query=latents,
            key=text_tokens,
            value=text_tokens
        )
        ...
        latents = latents + self.ff(self.ln(latents))  # FFN + residual
        return latents
       
class ImagineWorldModel(nn.Module):
    def __init__(self, 
                 config,
                ):
        super().__init__()
        ...
        # transfuserbackbone
        if config.latent:
            self._backbone = TransfuserBackboneLatent(config)
        else:
            self._backbone = TransfuserBackbone(config)
        self._bev_downscale = nn.Conv2d(512, config.tf_d_model, kernel_size=1)
        
        self.vlm_path = config.vlm_path
        ...
        #device = 'cpu'
        #import pdb;pdb.set_trace()
        self.unilatentmodel = ImagineWorldBackbone(
        ...
        #import pdb;pdb.set_trace()
        #print()
        self.tokens_downsample = PerceiverCrossAttention()
        self.cross_text_bev_align = BEVTextFusion()
    ...
    def encode_traj_into_ego_feat(self, ego_status_feat: torch.Tensor, init_trajectory_anchor: torch.Tensor, batch_size: int):
        ...
        Returns:
            ego_feat_fixed_anchor_ImagineWorld (torch.Tensor): Concatenated ego and trajectory features.
    ...
    def extract_trajectory_feature(self, features: Dict[str, torch.Tensor], targets=None, extract_trajectory_feature=None) -> Dict[str, Any]:
        """
        Part 1: extract_trajectory_featsure
        ...
        future_camera_feature = features['future_camera_feature']
        future_lidar_feature = features['future_lidar_feature']
        ...
        backbone_bev_feature, flatten_bev_feature_1 = self._process_backbone_features(
            camera_feature, lidar_feature
        )
        flatten_bev_feature = self.cross_text_bev_align(flatten_bev_feature_1, extract_trajectory_feature)
        #import pdb;pdb.set_trace()
        # Get ego status features
        
        future_backbone_bev_feature, future_flatten_bev_feature_1 = self._process_backbone_features(
            future_camera_feature, future_lidar_feature
        )
        #future_flatten_bev_feature = self.cross_text_bev_align(flatten_bev_feature, extract_trajectory_feature)
        #import pdb;pdb.set_trace()
        ...
        ego_feat_fixed_anchor_ImagineWorld, num_traj = self.encode_traj_into_ego_feat(ego_status_feat, init_trajectory_anchor, batch_size)

        # Optional offset prediction
        offset_dict = self._predict_offset(ego_feat_fixed_anchor_ImagineWorld, flatten_bev_feature)
        ...
        if self.is_eval:
            ...
            # training
            ego_feat_for_reward_network = ego_feat_fixed_anchor_ImagineWorld
        ...
        trajectory_outputs = {
            ...
            "future_flatten_bev_feature_1": future_flatten_bev_feature_1,
            "flatten_bev_feature_1": future_flatten_bev_feature_1
        }
    ...
    def process_trajectory_and_reward(self, features: Dict[str, torch.Tensor], targets=None, downsample_last_hidden_state=None) -> Dict[str, torch.Tensor]:
        #import pdb;pdb.set_trace()
        trajectory_outputs = self.extract_trajectory_feature(features, targets, downsample_last_hidden_state)
        #import pdb;pdb.set_trace()
        final_results = self.extract_reward_feature(trajectory_outputs, targets)
        #import pdb;pdb.set_trace()
        return final_results, trajectory_outputs
    ...
    def _process_future_backbone_features(self, camera_feature: torch.Tensor, lidar_feature: torch.Tensor):
        """
        Process the backbone network and extract BEV features.
        """
        _, backbone_bev_feature, _ = self._backbone(camera_feature, lidar_feature)
    ...
    def forward_test(self, features, targets=None) -> Dict[str, torch.Tensor]:
        ...
        #import pdb;pdb.set_trace()
        history_trajectory = features["history_trajectory"].cuda() # [4,3]
        ...
        pixel_values_cat = torch.cat(pixel_values_list, dim=0).cuda() # [9,3,448,448] 将图像切分为了patch
        #import pdb;pdb.set_trace()
        ...
        #import pdb;pdb.set_trace()
        outputs = self.unilatentmodel(pixel_values_cat, questions, num_patches_list=num_patches_list) # num_patches_list：9
        #import pdb;pdb.set_trace()
        last_hidden_state = outputs.hidden_states[-1]
        #import pdb;pdb.set_trace()
        downsample_last_hidden_state = self.tokens_downsample(last_hidden_state)
        #import pdb;pdb.set_trace()
        encoder_results,traj_results = self.process_trajectory_and_reward(features, targets, downsample_last_hidden_state)
        ...
        result["sim_rewards"] = sim_rewards
        #import pdb;pdb.set_trace()
        result["future_flatten_bev_feature_1"] = traj_results["future_flatten_bev_feature_1"]
        result["flatten_bev_feature_1"] = traj_results["flatten_bev_feature_1"]
        #if self.use_agent_loss:
