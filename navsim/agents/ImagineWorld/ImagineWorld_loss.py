...
def full_pairwise_rank_loss(scores, gt_scores, margin=0.0):
    """
    scores:    [B, K]
    gt_scores: [B, K]
    """
    ...
    gt_i = gt_scores.unsqueeze(2)
    gt_j = gt_scores.unsqueeze(1)

    # 只保留 gt_i > gt_j 的 pair
    pair_mask = (gt_i > gt_j).float()

    # RankNet: log(1 + exp(-(s_i - s_j)))
    diff = s_i - s_j - margin
    loss = torch.log1p(torch.exp(-diff)) * pair_mask
...
def compute_ImagineWorld_loss(
    ...
    '''
    dict_keys(['no_at_fault_collisions', 'drivable_area_compliance', 'driving_direction_compliance', 'ego_progress', 
    'time_to_collision_within_bound', 'comfort', 'final_scores', 'rewards', 'scores'])
    '''
    if config.train_stage == 'stage2':
        v_pred = predictions['v_pred']
        v_gt = predictions['v_gt']
        #import pdb;pdb.set_trace()
        loss_dict['latent_flow_matching_loss'] = F.mse_loss(v_pred, v_gt)
        return loss_dict
    if config.train_stage == 'stage3':
        NA_Pred = predictions['no_at_fault_collisions'].squeeze(2)
        DAC_Pred = predictions['drivable_area_compliance'].squeeze(2)
        DDC_Pred = predictions['driving_direction_compliance'].squeeze(2)
        EP_Pred = predictions['ego_progress'].squeeze(2)
        TTC_Pred = predictions['time_to_collision_within_bound'].squeeze(2)
        C_Pred = predictions['comfort'].squeeze(2)
        Fin_Score = predictions['final_scores'].squeeze(2)

        NA_Gt = predictions['rewards'][...,0]
        DAC_Gt = predictions['rewards'][...,1]
        DDC_Gt = predictions['rewards'][...,2]
        EP_Gt = predictions['rewards'][...,3]
        TTC_Gt = predictions['rewards'][...,4]
        C_Gt = predictions['rewards'][...,5]
        Fin_Gt = predictions['scores']

        na_loss = F.binary_cross_entropy_with_logits(NA_Pred, NA_Gt)
        dac_loss = F.binary_cross_entropy_with_logits(DAC_Pred, DAC_Gt)
        ddc_loss = F.binary_cross_entropy_with_logits(DDC_Pred, DDC_Gt)
        ep_loss = F.binary_cross_entropy_with_logits(EP_Pred, EP_Gt)
        ttc_loss = F.binary_cross_entropy_with_logits(TTC_Pred, TTC_Gt)
        c_loss = F.binary_cross_entropy_with_logits(C_Pred, C_Gt)
        Fin_loss = F.binary_cross_entropy_with_logits(Fin_Score, Fin_Gt)
        #import pdb;pdb.set_trace()
        loss_dict['na_loss'] = na_loss
        ...
        rank_loss = full_pairwise_rank_loss(Fin_Score, Fin_Gt, margin=0.0)

        loss_dict['Fin_loss'] = Fin_loss + 0.1 * rank_loss

        return loss_dict
    
    if config.train_stage == 'stage4' or config.train_stage == 'stage5':
        NA_Pred = predictions['no_at_fault_collisions'].squeeze(2)
        DAC_Pred = predictions['drivable_area_compliance'].squeeze(2)
        DDC_Pred = predictions['driving_direction_compliance'].squeeze(2)
        TTC_Pred = predictions['time_to_collision_within_bound'].squeeze(2)
        ...
        loss_dict['dac_loss'] = dac_loss
        loss_dict['ddc_loss'] = ddc_loss
        loss_dict['ttc_loss'] = ttc_loss * 1.5
        loss_dict['c_loss'] = c_loss
    ...
    if use_agent_loss:
        agent_cls_loss, agent_box_loss = compute_ImagineWorld_agent_loss(
...
def compute_ImagineWorld_agent_loss(
    gt_states, 
    gt_valid,
    pred_states,
    pred_logits,
