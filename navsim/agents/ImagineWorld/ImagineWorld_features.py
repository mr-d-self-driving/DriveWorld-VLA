...
class ImagineWorldFeatureBuilder(AbstractFeatureBuilder):
    def __init__(
            self,
            slice_indices=[3,11],
            config=None,
    ...
    def compute_features(self, agent_input: AgentInput) -> Dict[str, torch.Tensor]:
        ...
        cur_image_path = str(cameras[3].cam_f0.image_path)
        #future_image_path = str(cameras[11].cam_f0.image_path)
        #import pdb;pdb.set_trace()   
        #print(image_path) 
        cur_path_as_ordinals = [ord(char) for char in cur_image_path]
        
        cur_path_tensor = torch.tensor(cur_path_as_ordinals, dtype=torch.long)
        
        features["history_trajectory"] = history_trajectory#.cpu()
        features["high_command_one_hot"] = high_command_one_hot#.cpu()
        features["image_path_tensor"] = cur_path_tensor#.cpu()
        #print(path_tensor.shape)

        features["camera_feature"] = self._get_camera_feature(agent_input)
        features["future_camera_feature"] = self._get_future_camera_feature(agent_input)
        #import pdb;pdb.set_trace()
        features["lidar_feature"] = self._get_lidar_feature(agent_input)
        features["future_lidar_feature"] = self._get_lidar_future_feature(agent_input)
    ...
    def _get_future_camera_feature(self, agent_input: AgentInput) -> torch.Tensor:
        """
        Extract stitched camera from AgentInput
        :param agent_input: input dataclass
        :return: stitched front view image as torch tensor
        """
        #import pdb;pdb.set_trace()
        cameras = agent_input.cameras[11]
    ...
    def _get_lidar_feature(self, agent_input: AgentInput) -> torch.Tensor:
        ...
        lidar_pc = agent_input.lidars[3].lidar_pc[LidarIndex.POSITION].T
    ...
    def _get_lidar_future_feature(self, agent_input: AgentInput) -> torch.Tensor:
        """
        Compute LiDAR feature as 2D histogram, according to Transfuser
        :param agent_input: input dataclass
        :return: LiDAR histogram as torch tensors
        ...
        # only consider (x,y,z) & swap axes for (N,3) numpy array
        lidar_pc = agent_input.lidars[11].lidar_pc[LidarIndex.POSITION].T
