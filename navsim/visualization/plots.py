...
def plot_bev_frame(scene: Scene, token: str, frame_idx: int) -> Tuple[plt.Figure, plt.Axes]:
    """
    ...
    #Trajectory(poses)
    token_path = '/e2e-data/evad-tech-vla/liulin/WoTE/pkl_dir/' + token + 'top.pkl'
    import pickle
    from navsim.common.dataclasses import AgentInput, Trajectory, SensorConfig
    with open(token_path, "rb") as f:
        tensor = pickle.load(f)
    # pose_1 = tensor[0][10].cpu().numpy()
    #add_trajectory_to_bev_ax(ax, Trajectory(pose_1), TRAJECTORY_CONFIG["human"])
    add_trajectory_to_bev_ax(ax, human_trajectory, TRAJECTORY_CONFIG["human"])
    add_configured_bev_on_ax(ax, scene.map_api, scene.frames[frame_idx])
    #add_trajectory_to_bev_ax(ax, human_trajectory, TRAJECTORY_CONFIG["human"])
    configure_bev_ax(ax)
    configure_ax(ax) #222 ,9, 208, 206,43, 37, 2
    # 162, 160, 151, 135, 83,80
    # pose_1 = tensor[0][i].cpu().numpy()
    # #import pdb;pdb.set_trace()
    # add_trajectory_to_bev_ax(ax, Trajectory(pose_1), TRAJECTORY_CONFIG["human"])
    # add_configured_bev_on_ax(ax, scene.map_api, scene.frames[frame_idx])
